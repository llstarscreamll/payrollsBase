...
use App\Repositories\Criterias\SearchCountryCriteria;
 ...
 */
class CountryEloquentRepository extends BaseRepository implements CountryRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchCountryCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('country.form-labels.status_values.'.$item);
            })->all();
    }
