...
use App\Repositories\Criterias\SearchLegalPersonNatureCriteria;
use Illuminate\Support\Collection;

/**
 * Class LegalPersonNatureEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class LegalPersonNatureEloquentRepository extends BaseRepository implements LegalPersonNatureRepository
{
    /**
    ...
    {
        return LegalPersonNature::class;
    }
    ...
    {
        return $this->pushCriteria(new SearchLegalPersonNatureCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('legalPersonNature.form-labels.status_values.'.$item);
            })->all();
    }
