...
use App\Repositories\Criterias\SearchIdentityCardTypeCriteria;
use Illuminate\Support\Collection;

/**
 * Class IdentityCardTypeEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class IdentityCardTypeEloquentRepository extends BaseRepository implements IdentityCardTypeRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchIdentityCardTypeCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('identityCardType.form-labels.status_values.'.$item);
            })->all();
    }
