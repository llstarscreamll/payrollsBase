...
use App\Repositories\Criterias\SearchMunicipalityCriteria;
use Illuminate\Support\Collection;

/**
 * Class MunicipalityEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class MunicipalityEloquentRepository extends BaseRepository implements MunicipalityRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchMunicipalityCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('municipality.form-labels.status_values.'.$item);
            })->all();
    }
