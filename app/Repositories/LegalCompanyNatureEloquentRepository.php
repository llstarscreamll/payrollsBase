...
use App\Repositories\Criterias\SearchLegalCompanyNatureCriteria;
use Illuminate\Support\Collection;

/**
 * Class LegalCompanyNatureEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class LegalCompanyNatureEloquentRepository extends BaseRepository implements LegalCompanyNatureRepository
{
    /**
    ...
    {
        return LegalCompanyNature::class;
    }
    ...
    {
        return $this->pushCriteria(new SearchLegalCompanyNatureCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('legalCompanyNature.form-labels.status_values.'.$item);
            })->all();
    }
