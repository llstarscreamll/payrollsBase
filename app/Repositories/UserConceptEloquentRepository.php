...
use App\Repositories\Criterias\SearchUserConceptCriteria;
use Illuminate\Support\Collection;

/**
 * Class UserConceptEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class UserConceptEloquentRepository extends BaseRepository implements UserConceptRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchUserConceptCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('userConcept.form-labels.status_values.'.$item);
            })->all();
    }
