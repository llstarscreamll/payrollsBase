...
use App\Models\Bank;
use App\Repositories\Criterias\SearchBankCriteria;
use Illuminate\Support\Collection;

/**
 * Class BankEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class BankEloquentRepository extends BaseRepository implements BankRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchBankCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('bank.form-labels.status_values.'.$item);
            })->all();
    }
