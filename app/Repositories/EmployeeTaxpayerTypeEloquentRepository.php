...
use App\Repositories\Criterias\SearchEmployeeTaxpayerTypeCriteria;
use Illuminate\Support\Collection;

/**
 * Class EmployeeTaxpayerTypeEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class EmployeeTaxpayerTypeEloquentRepository extends BaseRepository implements EmployeeTaxpayerTypeRepository
{
    /**
    ...
    {
        return EmployeeTaxpayerType::class;
    }
    ...
    {
        return $this->pushCriteria(new SearchEmployeeTaxpayerTypeCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('employeeTaxpayerType.form-labels.status_values.'.$item);
            })->all();
    }
