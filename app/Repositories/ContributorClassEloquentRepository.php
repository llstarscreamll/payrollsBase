...
use App\Repositories\Criterias\SearchContributorClassCriteria;
use Illuminate\Support\Collection;

/**
 * Class ContributorClassEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class ContributorClassEloquentRepository extends BaseRepository implements ContributorClassRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchContributorClassCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('contributorClass.form-labels.status_values.'.$item);
            })->all();
    }
