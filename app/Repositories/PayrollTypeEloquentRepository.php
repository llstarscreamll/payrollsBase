...
use App\Repositories\Criterias\SearchPayrollTypeCriteria;
use Illuminate\Support\Collection;

/**
 * Class PayrollTypeEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class PayrollTypeEloquentRepository extends BaseRepository implements PayrollTypeRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchPayrollTypeCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('payrollType.form-labels.status_values.'.$item);
            })->all();
    }
