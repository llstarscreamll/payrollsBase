...
use App\Repositories\Criterias\SearchContributorTypeCriteria;
use Illuminate\Support\Collection;

/**
 * Class ContributorTypeEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class ContributorTypeEloquentRepository extends BaseRepository implements ContributorTypeRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchContributorTypeCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('contributorType.form-labels.status_values.'.$item);
            })->all();
    }
