...
use App\Repositories\Criterias\SearchBranchCriteria;
use Illuminate\Support\Collection;

/**
 * Class BranchEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class BranchEloquentRepository extends BaseRepository implements BranchRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchBranchCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('branch.form-labels.status_values.'.$item);
            })->all();
    }
