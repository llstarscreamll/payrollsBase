...
use App\Repositories\Criterias\SearchArlCompanyCriteria;
use Illuminate\Support\Collection;

/**
 * Class ArlCompanyEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class ArlCompanyEloquentRepository extends BaseRepository implements ArlCompanyRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchArlCompanyCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('arlCompany.form-labels.status_values.'.$item);
            })->all();
    }
