...
use App\Repositories\Criterias\SearchBranchOfficeCriteria;
use Illuminate\Support\Collection;

/**
 * Class BranchOfficeEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class BranchOfficeEloquentRepository extends BaseRepository implements BranchOfficeRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchBranchOfficeCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('branchOffice.form-labels.status_values.'.$item);
            })->all();
    }
