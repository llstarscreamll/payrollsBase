...
use App\Repositories\Criterias\SearchDepartmentCriteria;
use Illuminate\Support\Collection;

/**
 * Class DepartmentEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class DepartmentEloquentRepository extends BaseRepository implements DepartmentRepository
{
    /**
    ...
    {
        return $this->pushCriteria(new SearchDepartmentCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('department.form-labels.status_values.'.$item);
            })->all();
    }
