...
use App\Repositories\Criterias\SearchPilaPaymentOperatorCriteria;
use Illuminate\Support\Collection;

/**
 * Class PilaPaymentOperatorEloquentRepository
 *
 * @author Johan Alvarez <llstarscreamll@hotmail.com>
 */
class PilaPaymentOperatorEloquentRepository extends BaseRepository implements PilaPaymentOperatorRepository
{
    /**
    ...
    {
        return PilaPaymentOperator::class;
    }
    ...
    {
        return $this->pushCriteria(new SearchPilaPaymentOperatorCriteria($request))
    ...
    public function getEnumFieldSelectList(string $column)
    {
        return collect($this->getEnumValuesArray($column))
            ->map(function ($item, $key) {
                return $item = trans('pilaPaymentOperator.form-labels.status_values.'.$item);
            })->all();
    }
