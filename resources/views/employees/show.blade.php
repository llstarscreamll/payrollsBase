...
{{-- page title --}}
@section('title') {{trans('core::shared.views.show').trans('employee.module.name-singular')}} @stop
{{-- /page title --}}
...
{{-- heading --}}
@include('employees.partials.heading', ['small_title' => trans('core::shared.views.show')])
        ...
        <div class="col-xs-12 animated fadeInRight">
            ...
            <div class="ibox-content">
                ...
                {!! Form::model(
                    $employee,
                    [
                        'name' => 'show-employees-form',
                        'data-show' => ($show = true)
                    ]
                ) !!}

                    <div class='form-group col-sm-6 {{$errors->has('id') ? 'has-error' : ''}}'>
                        {!! Form::label('id', trans('employee.form-labels.id')) !!}
                        {!! Form::input('text', 'id', null, ['class' => 'form-control', isset($show) ? 'disabled' : '']) !!}
                    </div>

                    <div class="clearfix"></div>

                    @include('employees.partials.form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    @include('employees.partials.hidden-form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    <div class="form-group col-sm-6">
                        @if(auth()->user()->can('employees.edit'))
                            <a href="{{route('employees.edit', $employee->id)}}" class="btn btn-warning" role="button">
                        ...
                        @if(auth()->user()->can('employees.destroy'))
                            ...
                            {!! Form::open(['route' => ['employees.destroy', $employee->id], 'method' => 'DELETE', 'class' => 'form-inline display-inline']) !!}
                                ...
                                <button type="button"
                                        ...
                                        data-modalMessage="{{trans('core::shared.modal-delete-message', ['item' => $employee->name])}}"
