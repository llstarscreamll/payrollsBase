...
<div class="row tools">
    ...
    @if (array_get(Request::get(config('modules.core.app.search-fields-prefix', 'search')), 'trashed_records', null) != 'onlyTrashed' && auth()->user()->can('departments.destroy'))

    {{-- Formulario para borrar resgistros masivamente --}}
    {!! Form::open([
        'route' => ['departments.destroy', 0],
        'method' => 'DELETE',
        'id' => 'deletemanyForm',
    ...
    @endif
        ...
        @if(auth()->user()->can('departments.create'))
            ...
            {{--*******************************************************************************************************************************
                ...
                *******************************************************************************************************************************--}}
            <div class="display-inline" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('department.index-create-btn')}}">
                <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#create-form-modal">
                    <span class="glyphicon glyphicon-plus"></span>
                    <span class="sr-only">{{trans('department.index-create-btn')}}</span>
                </button>
            </div>

            {{-- Formulario de creación de registro --}}
            @include('departments.partials.index-create-form')
            ...
            <a id="create-departments-link" class="btn btn-default btn-sm" href="{!! route('departments.create') !!}" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('department.index-create-btn')}}">
                <span class="glyphicon glyphicon-plus"></span>
                <span class="sr-only">{{trans('department.index-create-btn')}}</span>
            </a>
                ******************************************************************************************************************************--}}
