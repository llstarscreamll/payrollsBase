...
{{-- page title --}}
@section('title') {{trans('core::shared.views.show').trans('userConcept.module.name-singular')}} @stop
{{-- /page title --}}
...
{{-- heading --}}
@include('userConcepts.partials.heading', ['small_title' => trans('core::shared.views.show')])
        ...
        <div class="col-xs-12 animated fadeInRight">
            ...
            <div class="ibox-content">
                ...
                {!! Form::model(
                    $userConcept,
                    [
                        'name' => 'show-user-concepts-form',
                        'data-show' => ($show = true)
                    ]
                ) !!}

                    <div class='form-group col-sm-6 {{$errors->has('id') ? 'has-error' : ''}}'>
                        {!! Form::label('id', trans('userConcept.form-labels.id')) !!}
                        {!! Form::input('text', 'id', null, ['class' => 'form-control', isset($show) ? 'disabled' : '']) !!}
                    </div>

                    <div class="clearfix"></div>

                    @include('userConcepts.partials.form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    @include('userConcepts.partials.hidden-form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    <div class="form-group col-sm-6">
                        @if(auth()->user()->can('user-concepts.edit'))
                            <a href="{{route('user-concepts.edit', $userConcept->id)}}" class="btn btn-warning" role="button">
                        ...
                        @if(auth()->user()->can('user-concepts.destroy'))
                            ...
                            {!! Form::open(['route' => ['user-concepts.destroy', $userConcept->id], 'method' => 'DELETE', 'class' => 'form-inline display-inline']) !!}
                                ...
                                <button type="button"
                                        ...
                                        data-modalMessage="{{trans('core::shared.modal-delete-message', ['item' => $userConcept->name])}}"
...
{{-- view scripts--}}
@section('scripts')
@include('userConcepts.partials.form-scripts')
@endsection()