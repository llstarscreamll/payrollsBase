...
<div class="row tools">
    ...
    @if (array_get(Request::get(config('modules.core.app.search-fields-prefix', 'search')), 'trashed_records', null) != 'onlyTrashed' && auth()->user()->can('branches.destroy'))

    {{-- Formulario para borrar resgistros masivamente --}}
    {!! Form::open([
        'route' => ['branches.destroy', 0],
        'method' => 'DELETE',
        'id' => 'deletemanyForm',
    ...
    @endif
        ...
        @if(auth()->user()->can('branches.create'))
            ...
            {{--*******************************************************************************************************************************
                ...
                *******************************************************************************************************************************--}}
            <div class="display-inline" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('branch.index-create-btn')}}">
                <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#create-form-modal">
                    <span class="glyphicon glyphicon-plus"></span>
                    <span class="sr-only">{{trans('branch.index-create-btn')}}</span>
                </button>
            </div>

            {{-- Formulario de creación de registro --}}
            @include('branches.partials.index-create-form')
            ...
            <a id="create-branches-link" class="btn btn-default btn-sm" href="{!! route('branches.create') !!}" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('branch.index-create-btn')}}">
                <span class="glyphicon glyphicon-plus"></span>
                <span class="sr-only">{{trans('branch.index-create-btn')}}</span>
            </a>
                ******************************************************************************************************************************--}}
