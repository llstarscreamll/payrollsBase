...
{{-- page title --}}
@section('title') {{trans('core::shared.views.show').trans('payrollType.module.name-singular')}} @stop
{{-- /page title --}}
...
{{-- heading --}}
@include('payrollTypes.partials.heading', ['small_title' => trans('core::shared.views.show')])
        ...
        <div class="col-xs-12 animated fadeInRight">
            ...
            <div class="ibox-content">
                ...
                {!! Form::model(
                    $payrollType,
                    [
                        'name' => 'show-payroll-types-form',
                        'data-show' => ($show = true)
                    ]
                ) !!}

                    <div class='form-group col-sm-6 {{$errors->has('id') ? 'has-error' : ''}}'>
                        {!! Form::label('id', trans('payrollType.form-labels.id')) !!}
                        {!! Form::input('text', 'id', null, ['class' => 'form-control', isset($show) ? 'disabled' : '']) !!}
                    </div>

                    <div class="clearfix"></div>

                    @include('payrollTypes.partials.form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    @include('payrollTypes.partials.hidden-form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    <div class="form-group col-sm-6">
                        @if(auth()->user()->can('payroll-types.edit'))
                            <a href="{{route('payroll-types.edit', $payrollType->id)}}" class="btn btn-warning" role="button">
                                <span class="glyphicon glyphicon-pencil"></span>
                                <span class="">{{trans('core::shared.edit-btn')}}</span>
                            </a>
                        @endif

                        @if(auth()->user()->can('payroll-types.destroy'))
                            {{-- Formulario para eliminar registro --}}
                            {!! Form::open(['route' => ['payroll-types.destroy', $payrollType->id], 'method' => 'DELETE', 'class' => 'form-inline display-inline']) !!}
                                ...
                                <button type="button"
                                        ...
                                        data-modalMessage="{{trans('core::shared.modal-delete-message', ['item' => $payrollType->name])}}"
