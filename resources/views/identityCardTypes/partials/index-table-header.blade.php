...
<tr class="header-row">
    ...
    <th class="id">
        {!! UISearch::sortLink('identity-card-types.index', trans('identityCardType.table-columns.id'), 'id') !!}
    </th>
    @endif
    @if(in_array('name', $selectedTableColumns))
    <th class="name">
        {!! UISearch::sortLink('identity-card-types.index', trans('identityCardType.table-columns.name'), 'name') !!}
    </th>
    @endif
