...
{{-- page title --}}
@section('title') {{trans('core::shared.views.show').trans('company.module.name-singular')}} @stop
{{-- /page title --}}
...
{{-- heading --}}
@include('companies.partials.heading', ['small_title' => trans('core::shared.views.show')])
        ...
        <div class="col-xs-12 animated fadeInRight">
            ...
            <div class="ibox-content">
                ...
                {!! Form::model(
                    $company,
                    [
                        'name' => 'show-companies-form',
                        'data-show' => ($show = true)
                    ]
                ) !!}

                    <div class='form-group col-sm-6 {{$errors->has('id') ? 'has-error' : ''}}'>
                        {!! Form::label('id', trans('company.form-labels.id')) !!}
                        {!! Form::input('text', 'id', null, ['class' => 'form-control', isset($show) ? 'disabled' : '']) !!}
                    </div>

                    <div class="clearfix"></div>

                    @include('companies.partials.form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    @include('companies.partials.hidden-form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    <div class="form-group col-sm-6">
                        @if(auth()->user()->can('companies.edit'))
                            <a href="{{route('companies.edit', $company->id)}}" class="btn btn-warning" role="button">
                        ...
                        @if(auth()->user()->can('companies.destroy'))
                            ...
                            {!! Form::open(['route' => ['companies.destroy', $company->id], 'method' => 'DELETE', 'class' => 'form-inline display-inline']) !!}
                                ...
                                <button type="button"
                                        ...
                                        data-modalMessage="{{trans('core::shared.modal-delete-message', ['item' => $company->name])}}"
