...
{{-- page title --}}
@section('title') {{trans('core::shared.views.show').trans('country.module.name-singular')}} @stop
{{-- /page title --}}
...
{{-- heading --}}
@include('countries.partials.heading', ['small_title' => trans('core::shared.views.show')])
        ...
        <div class="col-xs-12 animated fadeInRight">
            ...
            <div class="ibox-content">
                ...
                {!! Form::model(
                    $country,
                    [
                        'name' => 'show-countries-form',
                        'data-show' => ($show = true)
                    ]
                ) !!}

                    <div class='form-group col-sm-6 {{$errors->has('id') ? 'has-error' : ''}}'>
                        {!! Form::label('id', trans('country.form-labels.id')) !!}
                        {!! Form::input('text', 'id', null, ['class' => 'form-control', isset($show) ? 'disabled' : '']) !!}
                    </div>

                    <div class="clearfix"></div>

                    @include('countries.partials.form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    @include('countries.partials.hidden-form-fields', ['show' => ($show = true)])

                    <div class="clearfix"></div>

                    <div class="form-group col-sm-6">
                        @if(auth()->user()->can('countries.edit'))
                            <a href="{{route('countries.edit', $country->id)}}" class="btn btn-warning" role="button">
                        ...
                        @if(auth()->user()->can('countries.destroy'))
                            ...
                            {!! Form::open(['route' => ['countries.destroy', $country->id], 'method' => 'DELETE', 'class' => 'form-inline display-inline']) !!}
                                ...
                                <button type="button"
                                        ...
                                        data-modalMessage="{{trans('core::shared.modal-delete-message', ['item' => $country->name])}}"
...
{{-- view scripts--}}
@section('scripts')
@include('countries.partials.form-scripts')
@endsection()