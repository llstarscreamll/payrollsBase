...
<div class="row tools">
    ...
    @if (array_get(Request::get(config('modules.core.app.search-fields-prefix', 'search')), 'trashed_records', null) != 'onlyTrashed' && auth()->user()->can('pila-payment-operators.destroy'))

    {{-- Formulario para borrar resgistros masivamente --}}
    {!! Form::open([
        'route' => ['pila-payment-operators.destroy', 0],
        'method' => 'DELETE',
        'id' => 'deletemanyForm',
    ...
    @endif
        ...
        @if(auth()->user()->can('pila-payment-operators.create'))
            ...
            {{--*******************************************************************************************************************************
                ...
                *******************************************************************************************************************************--}}
            <div class="display-inline" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('pilaPaymentOperator.index-create-btn')}}">
                <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#create-form-modal">
                    <span class="glyphicon glyphicon-plus"></span>
                    <span class="sr-only">{{trans('pilaPaymentOperator.index-create-btn')}}</span>
                </button>
            </div>

            {{-- Formulario de creación de registro --}}
            @include('pilaPaymentOperators.partials.index-create-form')
            ...
            <a id="create-pila-payment-operators-link" class="btn btn-default btn-sm" href="{!! route('pila-payment-operators.create') !!}" role="button"  data-toggle="tooltip" data-placement="top" title="{{trans('pilaPaymentOperator.index-create-btn')}}">
                <span class="glyphicon glyphicon-plus"></span>
                <span class="sr-only">{{trans('pilaPaymentOperator.index-create-btn')}}</span>
            </a>
                ******************************************************************************************************************************--}}
